...
import (
	...
	"gitlab.cee.redhat.com/service/ocm-smtp-service/test/mocks"
	"io/ioutil"
	"log"
	"os"
	"path/filepath"
...
func (helper *Helper) RestURL(path string) string {
	...
	return fmt.Sprintf("%s://%s/api/ocm-smtp-service/v1%s", protocol, helper.AppConfig.Server.BindAddress, path)
}
...
func (helper *Helper) ClearAllTables() {
	helper.DeleteAll(&api.SMTPDetails{})
}
